"""Separately trained, person-calibrated quantile bounds; point model untouched."""
import numpy as np
import pandas as pd
...
def bounds(artifact, frame, point=None):
    ...
    if point is not None:
        # Enlargement cannot reduce coverage of the independently calibrated band.
        lower = np.minimum(lower, point)
        upper = np.maximum(upper, point)
    if not (np.isfinite(lower).all() and np.isfinite(upper).all()):
        raise ValueError('Non-finite compact prediction interval')
    return lower, upper


def calibration_adjustment(lower, upper, target, people, coverage=.90):
    scores = np.maximum.reduce([lower-target, target-upper, np.zeros(len(target))])
    person_scores = pd.Series(scores).groupby(np.asarray(people)).max().sort_values().to_numpy()
